...
Saída correta:
"itens_exame": [
   {{"descricao_ocr": "Hemograma Completo", "setor_sugerido": "laboratório", "confianca": 0.95}},
   {{"descricao_ocr": "Glicose", "setor_sugerido": "laboratório", "confianca": 0.95}},
   {{"descricao_ocr": "Creatinina", "setor_sugerido": "laboratório", "confianca": 0.95}}
]
...
▶ LÓGICA DE VETOR E ANCORAGEM HORIZONTAL (FORMULÁRIOS DENSOS)

Em formulários com múltiplas linhas próximas (ex: Painel de Agentes Infecciosos, Painel Molecular para HPV), marcações manuais podem estar ligeiramente deslocadas. Aplique obrigatoriamente:

PASSO 1 — MAPEAMENTO FÍSICO (faça ANTES de ler os textos):
  Localize a coordenada exata da tinta de cada marcação (centro físico do X ou traço).
  Liste mentalmente: "Há uma marca na posição vertical Y1, outra em Y2, etc."

PASSO 2 — ANCORAGEM HORIZONTAL:
  A partir do centro físico da marcação, trace uma linha imaginária horizontal reta para a direita.
  O texto cuja linha de base interceptar essa horizontal é o item selecionado.
  NUNCA ancore a marcação a um texto cuja linha de base esteja fisicamente ABAIXO do X.
  Exemplo: se o X está na altura da 1ª linha de "Clamídia / Gonococos", pertence a esse item,
  mesmo que o X pareça "solto" — NÃO deslize para a linha seguinte.

PASSO 3 — SUPRESSÃO DE VIÉS SEMÂNTICO:
  PROIBIDO deslocar mentalmente uma marcação física para outra opção por qualquer motivo semântico
  (ex: anotação manuscrita próxima, lógica clínica, contexto do caso).
  A marcação física no checkbox/círculo é a ÚNICA fonte de verdade.
  Se o posicionamento físico aponta para "HPV Alto Risco", extraia "HPV Alto Risco",
  mesmo que o texto manuscrito ao lado mencione outro exame.

PASSO 4 — CONFLITO ENTRE TEXTO MANUSCRITO E CHECKBOXES (REGRA DE FERRO):
  É extremamente comum médicos escreverem anotações à mão por cima ou ao lado de listas de opções.

  1. Se o checkbox/círculo de uma opção está VAZIO → a opção NÃO FOI SELECIONADA. Ponto final.
     O texto manuscrito solto NUNCA substitui a necessidade de marcação física no checkbox/círculo.

  2. NUNCA deduza que o médico "quis" pedir um exame apenas porque escreveu algo semelhante perto
     de uma opção. Intenção presumida não é marcação física.

  3. EXEMPLO CRÍTICO — Painel HPV:
     Se o médico escreveu "Genotipagem" à mão próximo ao formulário, mas o círculo de
     "HPV Quant 21 Tipos" está fisicamente VAZIO → PROIBIDO extrair este exame.
     Extraia SOMENTE as opções cujo traço de caneta (X ou preenchimento) intercepta
     diretamente a geometria do círculo/quadrado do checkbox.

     ┌──────────────────────────────────────────────────────────────┐
     │  ● Alto Risco           ← círculo com traço → EXTRAIR        │
     │  ● Baixo Risco          ← círculo com traço → EXTRAIR        │
     │  ○ HPV 28 Tipos         ← círculo vazio → NÃO EXTRAIR        │
     │  ○ HPV Quant 21 Tipos   ← círculo vazio → NÃO EXTRAIR        │
     │                  [manuscrito: "Genotipagem"] ← IGNORAR       │
     └──────────────────────────────────────────────────────────────┘
     Saída CORRETA: ["Alto Risco", "Baixo Risco"]
     Saída ERRADA:  ["Alto Risco", "Baixo Risco", "HPV Quant 21 Tipos"]

EXEMPLO DE ANCORAGEM:
┌─────────────────────────────────────────────────────────────────┐
│    X   HPV Alto Risco                                           │ → X alinhado → EXTRAIR
│        HPV Baixo Risco                                          │ → sem marca → NÃO extrair
│  X     HPV 28 Tipos                                             │ → X alinhado → EXTRAIR
│        HPV Quant 21 Tipos                                       │ → sem marca → NÃO extrair
└─────────────────────────────────────────────────────────────────┘
...
▶ DADOS CLÍNICOS E TEXTOS MANUSCRITOS (REGRA DE TRANSCRIÇÃO EXAUSTIVA)

É TERMINANTEMENTE PROIBIDO resumir, condensar, interpretar ou pular trechos de textos manuscritos
(campos como "Dados Complementares", "Informes Clínicos", "Observações", "Indicação") sob a justificativa
de caligrafia difícil.

1. TRANSCRIÇÃO INTEGRAL: Transcreva o texto do início ao fim, palavra por palavra, linha por linha.
   O esforço de decodificação deve ser MÁXIMO — tente todas as interpretações possíveis antes de desistir.

2. TRATAMENTO DE ILEGIBILIDADE: Se uma palavra específica for absolutamente impossível de decifrar,
   substitua APENAS essa palavra por "[ilegível]" e continue a transcrição do restante.
   NUNCA descarte uma frase inteira por causa de uma palavra ou trecho difícil.

3. ABREVIAÇÕES E SÍMBOLOS: Mantenha as abreviações médicas exatamente como escritas
   (ex: "c/", "s/", "colp", "AT", "ⓧ", "⊖") e reproduza símbolos quando reconhecíveis.

4. MÚLTIPLAS LINHAS: Se o texto manuscrito ocupa várias linhas, transcreva cada linha separada por " / "
   para indicar quebra de linha, preservando a sequência original.

EXEMPLO:
  Manuscrito (3 linhas):  "infecção recorrente"  /  "candida repetidos"  /  "AT⊖ colp prev ectopia perianfical"
  ✅ CORRETO: "infecção recorrente / candida repetidos / AT⊖ colp prev ectopia perianfical"
  ❌ ERRADO: "dados clínicos ilegíveis" ou omitir o campo

CAMPOS AFETADOS: dadosClinicos, indicacao, observacoes, e qualquer campo de texto livre.
...
3. REQUISIÇÃO/PEDIDO MÉDICO (Tipo mais comum):
   ...
   CAMPOS ADICIONAIS:
   • Dados clínicos / Observações (texto livre — transcrição integral obrigatória)
   • Data de nascimento ou idade do paciente
   • CPF (se presente)
   • Local de coleta (ex: "Ectocérvice", "Endocérvice", "Vagina", "Vulva", "Ânus", "Boca")
   • Última menstruação / UM (data manuscrita no campo "UM:" dos Informes Clínicos)
   • Indicação clínica (ex: "Rotina Preventiva", "Controle Após Tratamento", etc.)
   ...
   • Médico: "João Santos"
   • CRM: "54321/DF"
   • Local de coleta: ["Ectocérvice", "Endocérvice"]
   • Última menstruação: "15/03/2025"
...
{{
    ...
    "requisicao": {{
        ...
        "dadosClinicos": {{"valor": "transcrição integral do texto manuscrito", "fonte": "{imagem_nome}", "confianca": 0.80}},
        "indicacao": {{"valor": "ex: Rotina Preventiva | Alteração Citológica Prévia | Controle Após Tratamento", "fonte": "{imagem_nome}", "confianca": 0.85}},
        "localColeta": {{"valor": ["Ectocérvice", "Endocérvice"], "fonte": "{imagem_nome}", "confianca": 0.90}},
        "ultimaMenstruacao": {{"valor": "DD/MM/YYYY ou texto exato como escrito (ex: '15/4/26')", "fonte": "{imagem_nome}", "confianca": 0.85}},
        "itens_exame": [
         {{
...
2. NUNCA extraia um exame se o seu checkbox/círculo estiver vazio. Anotações manuscritas próximas a checkboxes vazios DEVEM SER IGNORADAS para fins de seleção de exame.
