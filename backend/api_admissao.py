...
try:
    ...
    print(f"âš ️ Módulo supabase_client não encontrado: {e}")

    print("âš ️ Execute: pip install supabase")
...
def buscar_dados_paciente_via_api(cod_requisicao):
    ...
    try:
        ...
        if resposta_resultado.get("dat", {}).get("sucesso") == 1:
            ...
            logger.warning(f"[API] âš ️ Falha ao buscar via requisicaoResultado")
...
def buscar_requisicao_correspondente(cod_requisicao):
    ...
    - Se começa com 0085 â†’ busca correspondente 0200
    ...
    try:
        ...
        # IMPORTANTE: O sufixo NÃƒO é necessariamente igual! (ex: 0085...005 vs 0200...006)

        # Por isso buscamos pelo paciente + data ao invés de sufixo exato

        # Ordenamos por proximidade do número da requisição para pegar a mais próxima
        ...
        if resultado:
            ...
            logger.info(f"[DB_SYNC] âœ… Requisição correspondente encontrada!")
...
def buscar_requisicao_correspondente_aplis(cod_requisicao):
    ...
    - Se começa com 0085 â†’ busca 0200 com mesmo sufixo
    ...
    try:
        ...
        # ESTRATÃ‰GIA: Buscar nos últimos 365 dias
...
def obter_id_convenio_default():
    ...
    Prioridade: 1) PARTICULAR, 2) Primeiro do cache ou banco

    Returns:

        int: ID do convenio default ou None se nao encontrar

    """

    if not CONVENIOS_CACHE:

        logger.warning("[Default] Cache de convenios vazio! Buscando no banco MySQL...")
        ...
        try:
            ...
            # Tentativa 1: nome contém ARTICULAR
            ...
            _cur.execute("""
                ...
                WHERE Inativo = 0 AND UPPER(NomConvenio) LIKE '%ARTICULAR%'
            ...
            if not row:
                ...
                # Tentativa 2: qualquer convenio ativo
            ...
            if not row:
                ...
                # Tentativa 3: absolutamente qualquer convenio
            ...
            if row:
                ...
                logger.error("[Default] Tabela fatconvenio esta vazia ou inacessivel")

        except Exception as _e:

            logger.error(f"[Default] Erro ao buscar convenio no banco: {_e}")
...
def obter_id_instituicao_default():
    ...
    Tenta o cache primeiro; se vazio, consulta o MySQL.

    Returns:

        int: ID da instituicao default ou None se nao encontrar
    ...
    if not INSTITUICOES_CACHE:
        ...
        try:
            ...
            # Tentativa 1: FontePagadora=1, nome contém ARTICULAR
            ...
            _cur.execute("""
                ...
                WHERE FontePagadora = 1 AND Inativo = 0
                  ...
                  AND UPPER(NomFantasia) LIKE '%ARTICULAR%'
            ...
            if not row:
                ...
                # Tentativa 2: FontePagadora=1, qualquer
            ...
            if not row:
                ...
                # Tentativa 3: qualquer instituicao ativa (sem filtro FontePagadora)
                ...
                _cur.execute("""
                    ...
                    WHERE Inativo = 0 AND NomFantasia IS NOT NULL
            ...
            if not row:
                ...
                # Tentativa 4: absolutamente qualquer registro da tabela
            ...
            logger.error(f"[Default] Erro ao buscar instituicao no banco: {_e}")
...
def _buscar_convenio_nome(id_convenio):
    ...
    try:
        ...
        logger.debug(f"[Helper] Total de convênios em cache: {len(CONVENIOS_CACHE)}")
...
def fazer_requisicao_aplis(cmd, dat, aplis_usuario=None, aplis_senha=None):
    ...
    # Usar credenciais fornecidas ou padrão do .env
...
def extrair_credenciais_usuario(request):
    ...
    Returns:
        ...
        tuple: (aplis_usuario, aplis_senha) ou (None, None) se não encontradas
...
def validar_e_corrigir_dados_cpf(dados_aplis, dados_sistema_antigo=None):
    ...
    try:
        ...
        if divergencias:
            ...
            logger.warning(f"[ValidarCPF] âš ️ DIVERGÃŠNCIAS ENCONTRADAS:")
...
def salvar_admissao_aplis(dados_admissao, aplis_usuario=None, aplis_senha=None):
    ...
    Args:
        ...
        dados_admissao (dict): Dados da admissão

        aplis_usuario (str, optional): Usuário do apLIS
    ...
    try:
        ...
        if resposta_direta.get("dat", {}).get("sucesso") == 1:
            ...
            if lista_direta and len(lista_direta) > 0:
                ...
                # ESTRATÃ‰GIA: requisicaoListar é PRIMÁRIO (rápido, sem atraso)
                ...
                # busca no banco MySQL pelo CodRequisicao ou pelo nome do paciente
                ...
                if not cod_paciente:
                    ...
                    try:
                        ...
                        if _row and _row.get("CodPaciente"):

                            cod_paciente = _row["CodPaciente"]
                ...
                if dados_paciente_api:
                    ...
                    logger.info(f"[BuscarIntegrado] âœ… Dados da API disponíveis")
                ...
                if id_fonte_pagadora:
                    ...
                    nome_fonte_pagadora = "Não informado"
...
def requisicoes_disponiveis():
    ...
    try:
        ...
        if not periodo_ini or not periodo_fim:
            ...
            # Padrao: dia anterior
            # Na segunda-feira (weekday=0), buscar desde sexta-feira para cobrir o fim de semana

            hoje = datetime.now()

            if hoje.weekday() == 0:
                # Segunda-feira: pegar so sexta (3 dias atras)
                sexta = hoje - timedelta(days=3)
                periodo_ini = sexta.strftime("%Y-%m-%d")
                periodo_fim = sexta.strftime("%Y-%m-%d")
            else:
        ...
        # StatusExame: 0=Aguarda Admissao, 1=Concluido, 2=Cancelado

        filtrar_status = dados.get("filtrarAguardaAdmissao", True)
...
def salvar_admissao():
    ...
    try:
        ...
        if 'idConvenio' not in dados or not dados.get('idConvenio'):
            ...
            if requisicao_existente and requisicao_existente.get('IdConvenio') and not fonte_do_ocr:
                ...
                if id_convenio_default:
                    ...
                    try:
                        ...
                        _conn_fc = pymysql.connect(**DB_CONFIG)
            ...
            # Se convênio foi informado E veio do OCR, logar a fonte
        ...
        if resultado.get("dat", {}).get("sucesso") == 1:
            ...
            try:
                ...
                logger.info(f"⏰ Data/Hora: {timestamp}")

                logger.info(f"ðŸ¥ Código Requisição: {cod_requisicao}")

                if cod_correspondente:

                    logger.info(f"ðŸ”— Requisição Correspondente: {cod_correspondente}")
                ...
                logger.info(f"   â€¢ Convênio: {nome_convenio} (ID: {convenio_log})")

                logger.info(f"   â€¢ Médico: {nome_medico} (ID: {medico_log})")
        ...
        logger.error(f"[SalvarAdmissao] ðŸ’¥ Erro exceção: {str(e)}")
        ...
        try:
            ...
            def validar_e_corrigir_data(data_str, campo_nome="data"):
                ...
                try:
                    ...
                    print(f"[OCR] âš ️ Erro ao validar data {data_str}: {e}")
                ...
                if cpf_limpo and len(cpf_limpo) == 11:
                    ...
                    if not id_paciente_existente:
                        ...
                        try:
                            ...
                            # Continuar mesmo com erro (não bloquear o OCR)
    ...
    try:
        ...
        if len(codigos_encontrados) > 1 and not dados_api:
            ...
            for codigo in codigos_encontrados:
                ...
                print(f"[CONSOLIDAR] ðŸ“ž Buscando requisição: {codigo}")
                ...
                try:
                    ...
                    if dados_req and dados_req.get('paciente'):
                        ...
                        print(f"[CONSOLIDAR] âœ… Requisição {codigo} encontrada!")
                    ...
                    print(f"[CONSOLIDAR] âŒ Erro ao buscar requisição {codigo}: {e}")
        ...
        print(f"\n[CONSOLIDAR] ðŸ”„ Verificando sincronização 0085 â†” 0200...")
...
def buscar_paciente():
    ...
    try:
        ...
        logger.error(f"[BUSCAR_PACIENTE] âŒ Exceção: {str(e)}")
